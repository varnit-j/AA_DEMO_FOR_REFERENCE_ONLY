...
class PaymentServiceUnitTests(unittest.TestCase):
    ...
    def test_service_health(self):
        ...
        print(f"SUCCESS Payment Service Health: Status {response.status_code}")
    ...
    def test_payment_health_api(self):
        ...
        print(f"SUCCESS Payment Health API: Status {response.status_code}")
    
    def test_payment_process_api(self):
        """Test payment process API"""
        response = self.session.get(f"{self.base_url}/api/payment/process/")
        self.assertIn(response.status_code, [200, 404, 405])
        print(f"SUCCESS Payment Process API: Status {response.status_code}")
    ...
    def test_stripe_config_api(self):
        ...
        self.assertIn(response.status_code, [200, 404, 405])
        print(f"SUCCESS Stripe Config API: Status {response.status_code}")

...
"""
SAGA Views for Backend Service
Handles ReserveSeat, ConfirmBooking, CancelSeat, CancelBooking
"""
import logging
import json
...
from .models import Ticket, Flight, User, Passenger
from .saga_event_dispatcher import event_dispatcher, SagaEvent

logger = logging.getLogger(__name__)

# In-memory storage for SAGA state (in production, use database)
saga_reservations = {}
...
def confirm_booking(request):
    ...
    try:
        ...
        # Simulate failure if requested
        if simulate_failure:
            logger.error(f"[SAGA] Simulated failure in Confirm